...
/**
 * @param {string} username - User del usuario.
 * @param {string} passw - Contraseña del usurio.
 * @param {string} correo - Correo electronico del usurio.
 * @param {string} telefono - Telefono del usuario.
 * @param {int} enviarInfo - Verifica si el usurio quiere informacion en su correo de la pagina.
*/
async function registrarUsuario(username, passw, correo, telefono, enviarInfo){
    
    try {
        var result = await sql.query(`SELECT id FROM usuarios WHERE username = '${username}'`)
        if(result[0][0]['id']){
            return 0
    }    
    }catch(error) {
        var results = await sql.query(`INSERT INTO usuarios (username, passw, correo, telefono, enviarInfo) VALUES ('${username}', '${passw}', '${correo}', '${telefono}', ${enviarInfo})`)
        const result = results[0]['affectedRows']
        if(result != 0){
            return 1
        }else{
...
// const a = async () => { 
//     var result = await sql.query(`SELECT id FROM usuarios WHERE username = 'Albasdasderto'`)
//     if(result[0][0]['id'])
//     {console.log(10)}
// }
a()
// [ [ { id: 1 } ], [ `id` INT NOT NULL PRIMARY KEY AUTO_INCREMENT ] ]
// [ [], [ `id` INT NOT NULL PRIMARY KEY AUTO_INCREMENT ] ]

/**
 * @param {string} username - Nombre de usuario de la pagina
 * @param {string} password - contraseña de usuario de la pagina
 */
async function iniciarSesionPag(username, password){
